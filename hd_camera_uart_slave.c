...
static void send_heartbeat(uint8_t ack) {
    unsigned char *protocol_data_out;
    uint32_t protocol_data_size_out;
    uint8_t slave_addr_in = PROTOCOL_SLAVE_ADDR_DEFAULT;
    uint8_t ack_number_in = ack + 1;
    uint8_t ret = hd_camera_protocol_cmd_heartbeat_resp_encode(&protocol_data_out, &protocol_data_size_out,
